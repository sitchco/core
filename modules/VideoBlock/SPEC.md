...
### Phase Zero: UIModal Content-Based Modal Support

UIModal currently requires a `Post` object to create a modal. The video block needs to create modals from arbitrary content (an iframe container) without a backing WordPress post.

UIModal requires a new method:

```php
// New method alongside existing loadModal(Post, ModalType)
$modal->loadModalContent(string $id, string $title, string $content, ModalType $type): ModalData
```

The `$title` parameter is required for modal accessibility (`aria-labelledby`). The `$id` is used for the `<dialog>` element ID and deep-link hash. This enables the video block (and future non-post-based modals) to create dialogs without a backing WordPress post. The rendered `<dialog>` structure and all JS behavior remain identical. The approach for refactoring `ModalData` (interface extraction, new subclass, or constructor overloading) is determined during phase zero implementation.
